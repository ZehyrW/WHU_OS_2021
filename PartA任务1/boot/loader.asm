...
.1:  
    ...
    jnc     .2
    inc     eax
    cmp     eax, PagesMapLen*8
    jne     .1
    push    szWrong; 没有足够空间使用
    call    DispStr
    add	    esp, 4
...
alloc_pages:
	...
	add ebx, eax
	mov edx, ebx    ; 储存PDE地址
	mov ebx, [es:edx] ; 获取PDE项内容

	test ebx, 0x00000001 ; 检查p位, 检查页表是否存在
	jnz .1
	;页表不存在，获取对应页表地址，修改PDE项
	;XCHG bx, bx
	mov ebx, cr3
	and ebx, 0xfffff000
	mov ebx, [es:ebx] ; 获取第一个页表地址
	and ebx, 0xfffff000
	shl eax, 10    ; 乘1K得到页表地址偏移
...
.1: ;页表存在, 计算对应的PTE地址
	...
	add ebx, eax
	mov edx, [es:ebx]
	test edx, 0x00000001 ; 检查p位, 检查页表项是否存在
	jnz .2
		;XCHG bx, bx
		push szUnused
		call DispStr
...
.2: ;擦除相应的物理地址
	;XCHG bx, bx
	and edx, 0xfffff000 ; 物理地址
	mov [es:ebx], edx   ; 清除PTE P位
	shr edx, 12         ; 得到物理地址对应MemoryUsage偏移
	btc [PagesMap], edx ; 清除占用位
	add ebx, 4 
...
_szReturn			db	0Ah, 0
_szWrong:		        db 	"Wrong",0Ah, 0
_szbegintest:		        db 	"test begin",0Ah, 0
_szalloc_ok:		        db 	"          alloc ok",0Ah, 0
_szfree_ok:		        db 	"          free ok",0Ah, 0
_szUnused:	        	db 	"The memory is not allocated",0Ah,0
; 变量
...
_PagesMap:   
 	times 32 db 0xff ; 0~0xffff occupied 0~1MB
        times 32 db 0x00 ; 0x100000~0x1fffff free space 1~2MB
	times 32 db 0xff ; 0x200000~0x2fffff occupied 2~3MB
	times 32 db 0x00 ; 0x300000~0x3fffff free space 3~4MB
	times 64 db 0xff ; 0x400000~0x5fffff occupied 4~6MB
PagesMapLen equ $ - _PagesMap
;
;; 保护模式下使用这些符号
...
szWrong 	        equ     BaseOfLoaderPhyAddr +_szWrong 
szUnused	        equ     BaseOfLoaderPhyAddr +_szUnused 
szalloc_ok		equ	BaseOfLoaderPhyAddr +_szalloc_ok 
szfree_ok		equ	BaseOfLoaderPhyAddr +_szfree_ok 
szbegintest		equ	BaseOfLoaderPhyAddr +_szbegintest 
...
PagesMap                equ     BaseOfLoaderPhyAddr +_PagesMap 
